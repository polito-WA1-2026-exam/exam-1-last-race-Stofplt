...
- The `segments.path` value stores the SVG path for that directed segment. Opposite segments follow the same physical connection but store the path in opposite drawing order, so execution animations can follow the selected travel direction.
